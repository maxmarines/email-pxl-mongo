...
class MongodbPersistenceLayer extends Pxl.PersistenceLayerBase {
    ...
    connect(uri = "mongodb://localhost:27017/test", connectionOptions = {}) {
        if (this.db) {
            return new Promise((resolve, reject) => {
                throw new Error(
                    "Database connection is already established. Please do not call PxlMongodb.connect(...) twice."
                );
            });
        }
        ...
        return mongodb.MongoClient.connect(uri, connectionOptions)
            ...
            .then(() => {
                return this.db
                    .collection(this.collectionPxls)
                    .ensureIndex(
                        { pxl: 1 },
                        { unique: true, background: true, name: "pxl_1" }
                    );
            })
            .then(() => {
                return this.db
                    .collection(this.collectionLinks)
                    .ensureIndex(
                        { linkId: 1 },
                        { unique: true, background: true, name: "linkId_1" }
                    );
            })
            .then(() => {
    ...
    disconnect() {
        if (!this.db) {
            return new Promise((resolve, reject) => {
                throw new Error(
                    "Database connection is not established. Please call PxlMongodb.connect(...) first."
                );
            });
        }
    ...
    checkAndAddLink(linkId, link, _skipExistingLinkCheck = false) {
        ...
        if (
            !_skipExistingLinkCheck &&
            this.alwaysShortenWithNewLinkId === false
        ) {
            return this.db
                .collection(this.collectionLinks)
                .findOne({ link })
